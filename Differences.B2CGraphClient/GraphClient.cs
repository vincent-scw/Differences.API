...
namespace Differences.B2CGraphClient
{
    public class GraphClient
    {
        ...
        private readonly B2CGraphQL _configuration;
        private readonly AuthenticationContext authContext;
        private readonly ClientCredential credential;

        public GraphClient(IOptions<B2CGraphQL> options)
        {
            _configuration = options.Value;

            // The AuthenticationContext is ADAL's primary class, in which you indicate the direcotry to use.
            this.authContext = new AuthenticationContext("https://login.microsoftonline.com/" + _configuration.Tenant);

            // The ClientCredential is where you pass in your client_id and client_secret, which are 
            // provided to Azure AD in order to receive an access_token using the app's identity.
            this.credential = new ClientCredential(_configuration.ClientId, _configuration.ClientSecret);
        }
        ...
        private async Task<string> SendGraphDeleteRequest(string api)
        {
            ...
            string url = AadGraphEndpoint + _configuration.Tenant + api + "?" + AadGraphVersion;
        ...
        public async Task<string> SendGraphGetRequest(string api, string query)
        {
            ...
            HttpClient http = new HttpClient();
            string url = $"https://graph.windows.net/{_configuration.Tenant}{api}?{AadGraphVersion}";
